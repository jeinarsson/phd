...
\end{align}
Again, if we solve \Eqnref{nsintrodimless2} \emph{exactly} and transform the result back to dimensionful variables we find the exact solution to \Eqnref{nsintro}. But the asymptotic solution to \Eqnref{nsintrodimless2} is qualitatively different from that of \Eqnref{nsintrodimless}. The choice of dimensionless variables embodies the physical assumptions that are effected by the perturbation theory. For the force on a translating sphere, this example of a change in time scaling is responsible for the difference between the Stokes and the Stokes-Boussinesq forces in Eqs.~\eqnref{stokesforce} and \eqnref{FSB}.
